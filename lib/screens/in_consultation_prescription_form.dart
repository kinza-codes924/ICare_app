...
class _InConsultationPrescriptionFormState
  ...
  Future<void> _completePrescription() async {
    ...
    if (widget.consultationId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Consultation ID missing — cannot complete prescription',
          ),
  ...
  Map<String, dynamic> _buildMinimalPayload(EnhancedPrescription rx) {
    ...
    if (rx.soapNotes != null) {
      ...
      if (s.subjective.isNotEmpty ||
          s.objective.isNotEmpty ||
          s.assessment.isNotEmpty ||
          s.plan.isNotEmpty) {
        payload['soapNotes'] = {
    ...
    if (rx.diagnoses.isNotEmpty) {
      payload['diagnoses'] = rx.diagnoses
          .map((d) => {'icd10Code': d.icd10Code, 'diagnosis': d.diagnosis})
          .toList();
    } else {
      payload['diagnoses'] = [];
    }
    ...
    if (rx.medicines.isNotEmpty) {
      payload['medicines'] = rx.medicines
          .map(
            (m) => {
    ...
    if (rx.labTests.isNotEmpty) {
      payload['labTests'] = rx.labTests
          .map((t) => {'testName': t.testName, 'isUrgent': t.isUrgent})
          .toList();
    } else {
      payload['labTests'] = [];
    }
    ...
    if (rx.lifestyleAdvice != null) {
      ...
      final hasContent =
          (la.diet?.recommendations.isNotEmpty == true) ||
            ...
            children: [
              ...
              if (widget.appointment != null)
                _accordion(
                  ...
                  subtitle: _patientHistoryId != null
                      ? 'Completed ✓'
  ...
  Future<void> _openHistoryForm() async {
    ...
    if (_patientHistoryId != null) {
      try {
        existingHistory = await _consultationService.getHistoryByConsultation(
          widget.consultationId,
        );
        existingHistoryId =
            existingHistory?['_id']?.toString() ?? _patientHistoryId;
      } catch (_) {}
    }
      ...
      children: [
                    ...
                    (code) => InkWell(
                      onTap: () {
                        ...
                        if (desc.isNotEmpty) {
                          setState(() {
                            _diagnoses.add(
                              DiagnosisItem(diagnosis: desc, icd10Code: icd),
                            );
                            _diagSearchController.clear();
                            _diagSearchResults = [];
                          });
        ...
        if (_diagnoses.isEmpty)
          _emptyState(
            ...
            'Search and add diagnoses above',
          )
        else
  ...
  Future<void> _searchICD(String query) async {
    ...
    try {
      ...
      if (mounted)
        setState(() {
          _diagSearchResults = List<Map<String, dynamic>>.from(results);
          _diagSearching = false;
        });
    } catch (_) {
  ...
  Widget _buildMedicationsContent() {
    ...
    final commonSuggestions = searchText.isEmpty
        ? <String>[]
        : _commonMeds
              .where((m) => m.toLowerCase().contains(searchText))
              .toList();
    final drugResults = searchText.isEmpty
        ? <DrugFormula>[]
    ...
    return Column(
      ...
      children: [
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: _medSearchController.text.isNotEmpty
                ? IconButton(
                    icon: const Icon(Icons.clear_rounded, size: 18),
                    onPressed: () =>
                        setState(() => _medSearchController.clear()),
                  )
                : null,
            filled: true,
        ...
        if (_medicines.isEmpty)
          _emptyState(
            Icons.medication_outlined,
            'Search above and tap a medicine to add it',
          )
        else
          ...List.generate(_medicines.length, (index) {
            final m = _medicines[index];
            _medDoseControllers.putIfAbsent(
              index,
              () => TextEditingController(text: m.dose),
            );
            _medNotesControllers.putIfAbsent(
              index,
              () => TextEditingController(text: m.notes ?? ''),
            );
            _medDurationControllers.putIfAbsent(
              index,
                      ...
                      Expanded(
                        ...
                        child: _compactField(
                          ...
                          onChanged: (v) => setState(
                            () => _medicines[index] = PrescriptionMedicine(
                              medicineName: m.medicineName,
                              dose: v,
                              formType: m.formType,
                      ...
                      Expanded(
                        ...
                        child: _compactDropdown<MedicineFormType>(
                          ...
                          items: [
                            ...
                            MedicineFormType.liquid,
                            MedicineFormType.drops,
                            MedicineFormType.injection,
                            MedicineFormType.cream,
                            MedicineFormType.inhaler,
                          ],
                          label: (f) {
                            switch (f) {
                              case MedicineFormType.tablet:
                                return 'Tab';
                              case MedicineFormType.capsule:
                                return 'Cap';
                              case MedicineFormType.liquid:
                                return 'Syrup';
                              case MedicineFormType.drops:
                                return 'Drops';
                              case MedicineFormType.injection:
                                return 'Inj';
                              case MedicineFormType.cream:
                                return 'Cream';
                              case MedicineFormType.inhaler:
                                return 'Inhaler';
                              default:
                                return 'Other';
                            }
  ...
  Widget _buildLabTestsContent() {
    ...
    final filteredTests = filterText.isEmpty
        ...
        : CommonLabTests.tests
              .where((t) => t.toLowerCase().contains(filterText))
              .toList();
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: _labSearchController.text.isNotEmpty
                ? IconButton(
                    icon: const Icon(Icons.clear_rounded, size: 18),
                    onPressed: () =>
                        setState(() => _labSearchController.clear()),
                  )
                : null,
            filled: true,
                  ...
                  (t) => Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          onTap: () => setState(
                            () => _labTests.removeWhere(
                              (x) => x.testName == t.testName,
                            ),
                          ),
                          child: const Icon(
                            Icons.close_rounded,
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: LifestyleAdviceTemplates.dietTemplates
              .map(
                (t) => ActionChip(
        ...
        _lifestyleSection(
          ...
          child: Column(
            ...
            children: [
              _lifestyleField(
                'Recommendations',
                advice?.diet?.recommendations ?? '',
                (v) => _updateLifestyleAdvice(dietRec: v),
              ),
              const SizedBox(height: 10),
              _lifestyleField(
                'Foods to Avoid',
                advice?.diet?.foodsToAvoid.join(', ') ?? '',
                (v) => _updateLifestyleAdvice(dietAvoid: v),
              ),
              const SizedBox(height: 10),
              _lifestyleField(
                'Foods to Include',
                advice?.diet?.foodsToInclude.join(', ') ?? '',
                (v) => _updateLifestyleAdvice(dietInclude: v),
              ),
              const SizedBox(height: 10),
              Row(
                children: [
                  Expanded(
                    child: _lifestyleField(
                      'Meal Timing',
                      advice?.diet?.mealTiming ?? '',
                      (v) => _updateLifestyleAdvice(mealTiming: v),
                    ),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: _lifestyleField(
                      'Hydration',
                      advice?.diet?.hydration ?? '',
                      (v) => _updateLifestyleAdvice(hydration: v),
                    ),
                  ),
                ],
              ),
          ...
          child: Column(
            ...
            children: [
              Wrap(
                spacing: 8,
                runSpacing: 6,
                children: LifestyleAdviceTemplates.exerciseTemplates
                    .map(
                      (t) => ActionChip(
                        label: Text(
                          t['name'] as String,
              ...
              Row(
                children: [
                  Expanded(
                    child: _lifestyleField(
                      'Type',
                      advice?.exercise?.type ?? '',
                      (v) => _updateLifestyleAdvice(exerciseType: v),
                    ),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: _lifestyleField(
                      'Frequency',
                      advice?.exercise?.frequency ?? '',
                      (v) => _updateLifestyleAdvice(exerciseFreq: v),
                    ),
                  ),
                ],
              ),
              ...
              Row(
                children: [
                  Expanded(
                    child: _lifestyleField(
                      'Duration',
                      advice?.exercise?.duration ?? '',
                      (v) => _updateLifestyleAdvice(exerciseDur: v),
                    ),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: _lifestyleField(
                      'Intensity',
                      advice?.exercise?.intensity ?? '',
                      (v) => _updateLifestyleAdvice(exerciseIntensity: v),
                    ),
                  ),
                ],
              ),
              ...
              _lifestyleField(
                'Precautions',
                advice?.exercise?.precautions.join(', ') ?? '',
                (v) => _updateLifestyleAdvice(exercisePrecautions: v),
              ),
            ],
          ),
        ),
          ...
          child: Column(
            ...
            children: [
              Wrap(
                spacing: 8,
                runSpacing: 6,
                children: LifestyleAdviceTemplates.sleepTemplates
                    .map(
                      (t) => ActionChip(
                        label: Text(
                          t['name'] as String,
              ...
              Row(
                children: [
                  Expanded(
                    child: _lifestyleField(
                      'Recommended Hours',
                      advice?.sleep?.recommendedHours ?? '',
                      (v) => _updateLifestyleAdvice(sleepHours: v),
                    ),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: _lifestyleField(
                      'Sleep Schedule',
                      advice?.sleep?.sleepSchedule ?? '',
                      (v) => _updateLifestyleAdvice(sleepSchedule: v),
                    ),
                  ),
                ],
              ),
              ...
              _lifestyleField(
                'Sleep Hygiene Tips',
                advice?.sleep?.sleepHygieneTips.join(', ') ?? '',
                (v) => _updateLifestyleAdvice(sleepTips: v),
              ),
            ],
          ),
        ),
          ...
          child: Column(
            ...
            children: [
              Wrap(
                spacing: 8,
                runSpacing: 6,
                children: LifestyleAdviceTemplates.stressTemplates
                    .map(
                      (t) => ActionChip(
                        label: Text(
                          t['name'] as String,
              ...
              _lifestyleField(
                'Techniques',
                advice?.stress?.techniques.join(', ') ?? '',
                (v) => _updateLifestyleAdvice(stressTechniques: v),
              ),
              const SizedBox(height: 10),
              _lifestyleField(
                'Recommendations',
                advice?.stress?.recommendations ?? '',
                (v) => _updateLifestyleAdvice(stressRec: v),
              ),
            ],
          ),
        ),
  ...
  void _updateLifestyleAdvice({
    ...
    String? exercisePrecautions,
    String? sleepHours,
    String? sleepSchedule,
    String? sleepTips,
    String? stressTechniques,
    String? stressRec,
    String? otherAdvice,
  }) {
    ...
    setState(() {
      _lifestyleAdvice = LifestyleAdvice(
        ...
        diet: DietAdvice(
          ...
          foodsToAvoid: dietAvoid != null
              ? dietAvoid
                    .split(',')
                    .map((s) => s.trim())
                    .where((s) => s.isNotEmpty)
                    .toList()
              : existing?.diet?.foodsToAvoid ?? [],
          foodsToInclude: dietInclude != null
              ? dietInclude
                    .split(',')
                    .map((s) => s.trim())
                    .where((s) => s.isNotEmpty)
                    .toList()
              : existing?.diet?.foodsToInclude ?? [],
        ...
        exercise: ExerciseAdvice(
          ...
          precautions: exercisePrecautions != null
              ? exercisePrecautions
                    .split(',')
                    .map((s) => s.trim())
                    .where((s) => s.isNotEmpty)
                    .toList()
              : existing?.exercise?.precautions ?? [],
        ),
        sleep: SleepAdvice(
          recommendedHours:
              sleepHours ?? existing?.sleep?.recommendedHours ?? '',
          sleepSchedule: sleepSchedule ?? existing?.sleep?.sleepSchedule ?? '',
          sleepHygieneTips: sleepTips != null
              ? sleepTips
                    .split(',')
                    .map((s) => s.trim())
                    .where((s) => s.isNotEmpty)
                    .toList()
              : existing?.sleep?.sleepHygieneTips ?? [],
        ),
        stress: StressManagement(
          techniques: stressTechniques != null
              ? stressTechniques
                    .split(',')
                    .map((s) => s.trim())
                    .where((s) => s.isNotEmpty)
                    .toList()
              : existing?.stress?.techniques ?? [],
          recommendations: stressRec ?? existing?.stress?.recommendations ?? '',
        ),
        smoking: existing?.smoking,
        alcohol: existing?.alcohol,
        weight: existing?.weight,
        otherAdvice: otherAdvice != null
            ? otherAdvice.split('\n').where((s) => s.isNotEmpty).toList()
            : existing?.otherAdvice ?? [],
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<ReferralType>(
                ...
                items: const [
                  DropdownMenuItem(
                    value: ReferralType.none,
                    child: Text('No Referral'),
                  ),
                  DropdownMenuItem(
                    value: ReferralType.emergency,
                    child: Text('Refer to Emergency'),
                  ),
                  DropdownMenuItem(
                    value: ReferralType.hospital,
                    child: Text('Refer to Hospital'),
                  ),
                  DropdownMenuItem(
                    value: ReferralType.specialist,
                    child: Text('Refer to Specialist'),
                  ),
                ],
                onChanged: (v) => setState(() {
              ...
              if ((ref?.referralType ?? ReferralType.none) !=
                  ReferralType.none) ...[
                const SizedBox(height: 12),
                TextField(
                  maxLength: 500,
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<FollowUpDuration>(
                ...
                items: FollowUpDuration.values
                    .map(
                      (d) => DropdownMenuItem(value: d, child: Text(d.display)),
                    )
                    .toList(),
                onChanged: (v) => setState(() {
                  _referralFollowUp = ReferralFollowUp(
  ...
  Widget _buildCoursesContent() {
    ...
    return Column(
      ...
      children: [
        const Text(
          'Assign Health Courses to Patient',
          style: TextStyle(
            fontWeight: FontWeight.w700,
            fontSize: 13,
            color: Color(0xFF475569),
        ...
        if (_assignedCourses.isNotEmpty) ...[
          ...
          Text(
            '${_assignedCourses.length} course${_assignedCourses.length > 1 ? 's' : ''} assigned',
            style: const TextStyle(
              color: Color(0xFF06B6D4),
              fontWeight: FontWeight.w700,
              fontSize: 13,
...
class _PharmaPickerSheet extends StatefulWidget {
  ...
  final void Function(String medicineName, String? strength, String? form)
  onAdd;
              ...
              children: brands.map((brand) {
                ...
                return GestureDetector(
                  onTap: () => setState(() {
                    ...
                    _strength = brand.strengths.isNotEmpty
                        ? brand.strengths.first
                        : null;
                  }),
            ...
            if (_brand != null) ...[
              ...
              Row(
                children: [
                  ElevatedButton.icon(
                    onPressed: () =>
                        widget.onAdd(_buildMedicineName(), _strength, _form),
                    icon: const Icon(
