...
class CertificateVerificationPage extends StatefulWidget {
  ...
  const CertificateVerificationPage({super.key, this.initialCode});
  ...
  Future<void> _verifyCertificate() async {
    ...
    if (code.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please enter certificate code'),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      ...
                      label: Text(
                        _loading ? 'Verifying...' : 'Verify Certificate',
                      ),
                  ...
                  child: Column(
                    children: [
                      ...
                      if (_result!['valid'] == true &&
                          _result!['certificate'] != null) ...[
                        const SizedBox(height: 24),
                        const Divider(),
                        const SizedBox(height: 16),
                        _buildInfoRow(
                          'Student',
                          _result!['certificate']['studentName'] ?? 'N/A',
                        ),
                        const SizedBox(height: 12),
                        _buildInfoRow(
                          'Course',
                          _result!['certificate']['courseName'] ?? 'N/A',
                        ),
                        const SizedBox(height: 12),
                        _buildInfoRow(
                          'Issued Date',
                          _result!['certificate']['issuedAt'] ?? 'N/A',
                        ),
                        const SizedBox(height: 12),
                        _buildInfoRow(
                          'Certificate ID',
                          _result!['certificate']['certificateId'] ?? 'N/A',
                        ),
                      ],
                    ],
                  ),
