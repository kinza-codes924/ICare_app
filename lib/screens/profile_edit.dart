...
class _ProfileEditScreenState extends ConsumerState<ProfileEditScreen> {
  ...
  void initState() {
    ...
    debugPrint(
      '🟦 [EditProfile] initState: user=${user?.name}, specialization=${user?.specialization}, role=${ref.read(authProvider).userRole}',
    );
    if (user != null) {
      ...
      _selectedBloodGroup = (user.bloodGroup?.isNotEmpty == true)
          ? user.bloodGroup
          : null;
      ...
      if (user.existingConditions != null &&
          user.existingConditions!.isNotEmpty) {
    ...
    if (role == 'Doctor') {
      ...
      if (user != null) {
        ...
        if (user.conditionsTreated != null &&
            user.conditionsTreated!.isNotEmpty) {
    ...
    if (result['success'] == true && result['user'] is Map) {
      ...
      setState(() {
        ...
        if (g.toLowerCase() == 'male')
          ...
          _selectedGender = 'Other';
        final conds = (u['existingConditions'] ?? u['existing_conditions'])
            ?.toString();
  ...
  void _populateEmergencyContacts(List<Map<String, String>> contacts) {
    ...
    for (int i = 0; i < contacts.length; i++) {
      ...
      _emergencyContacts[i]['relation']!.text =
  ...
  Future<void> _loadDoctorProfile() async {
    ...
    if (result['success'] == true && result['doctor'] != null) {
      ...
      debugPrint(
        '🟦 [EditProfile] doctor doc: name=${doc['name']}, phone=${doc['phoneNumber']}, spec=${doc['specialization']}, conds=${doc['conditionsTreated']}',
      );
      setState(() {
      ...
      debugPrint(
        '🔴 [EditProfile] _loadDoctorProfile FAILED: ${result['message']}',
      );
    }
  }
  ...
  void _handleUpdate() async {
    ...
    try {
      ...
      debugPrint(
        '🟩 [EditProfile] _handleUpdate: role=$role, isPatient=$isPatient, spec=$_selectedSpecialization, name=${nameController.text}',
      );
      ...
      if (isDoctor &&
        ...
        debugPrint(
          '🟩 [EditProfile] updateDoctorSpecialization result: $specResult',
        );
      }
      ...
      final result = await _userService.updateProfile(
        ...
        healthGoals: isPatient
            ? (_selectedGoals.isEmpty ? null : _selectedGoals.join(', '))
            : null,
      ...
      debugPrint(
        '🟩 [EditProfile] updateProfile result: success=${result['success']}, msg=${result['message']}',
      );
      if (result['success']) {
        ...
        final updatedUser = app_user.User(
          ...
          gender: isPatient
              ? (_selectedGender ?? existingUser.gender)
              : existingUser.gender,
          age: isPatient
          ...
          cnic: isPatient
              ? (cnicController.text.trim().isNotEmpty
                    ? cnicController.text.trim()
                    : existingUser.cnic)
              : existingUser.cnic,
          ...
          bloodGroup: isPatient
              ? (_selectedBloodGroup ?? existingUser.bloodGroup)
        ...
        debugPrint(
          '🟩 [EditProfile] updatedUser: name=${updatedUser.name}, spec=${updatedUser.specialization}, conds=${updatedUser.conditionsTreated}',
        );
        await ref.read(authProvider.notifier).setUser(updatedUser);
        debugPrint(
          '🟩 [EditProfile] setUser done. State user spec=${ref.read(authProvider).user?.specialization}',
        );
                          ...
                          if (isDoctor) ...[
                            ...
                            Wrap(
                              ...
                              children: _doctorConditions.map((c) {
                                final sel = _selectedDoctorConditions.contains(
                                  c,
                                );
                          ...
                          if (isPatient) ...[
                            ...
                            if (_heightUnit == 'cm')
                              ...
                              _buildDropdown<String>(
                                ...
                                items: List.generate(
                                  71,
                                  (i) => (1.40 + i * 0.01).toStringAsFixed(2),
                                ),
