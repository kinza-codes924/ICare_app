...
class InstructorCourseContentScreen extends StatefulWidget {
  ...
  State<InstructorCourseContentScreen> createState() =>
      InstructorCourseContentScreenState();
}

class InstructorCourseContentScreenState
  ...
  Future<void> _loadCourse() async {
    ...
    try {
      final results = await Future.wait([
        ...
        _lmsService
            .getCourseAssignments(widget.courseId)
            .catchError((_) => <dynamic>[]),
        _lmsService
            .getCourseQuizzes(widget.courseId)
            .catchError((_) => <dynamic>[]),
        _lmsService
            .getCourseSessions(widget.courseId)
            .catchError((_) => <dynamic>[]),
      ]);
      if (mounted) {
        setState(() {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => LmsLiveSessionScreen(
          sessionId:
              '', // empty — backend will create a fresh session with a real _id
  ...
  Future<void> _addModule() async {
    ...
    if (result != null) {
      ...
      if (mounted)
        setState(() {
          _course = {...?_course, 'modules': modules};
        });
  ...
  Future<void> _editModule(int index) async {
    ...
    final result = await showDialog<Map<String, dynamic>>(
      context: context,
      builder: (context) =>
          _ModuleDialog(module: modules[index], courseId: widget.courseId),
    );
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Course type set to ${type == 'pragmatic' ? 'Pragmatic (Timeline)' : 'Self-paced'}',
            ),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              value
                  ? 'Course now shows as Free to students'
                  : 'Course now shows its real price',
            ),
            backgroundColor: Colors.green,
          ),
        );
  ...
  void _showEditThumbnail() {
    final urlCtrl = TextEditingController(
      text:
          _course?['thumbnail']?.toString() ??
          _course?['thumbnail_url']?.toString() ??
          '',
    );
          ...
          child: Column(
            ...
            children: [
              ...
              OutlinedButton.icon(
                onPressed: uploading
                    ...
                    : () async {
                        ...
                        try {
                          final sigRes = await ApiService().get(
                          ...
                          final folder = (sig['folder'] ?? 'icare/thumbnails')
                              .toString();
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: IconButton(
                    ...
                    onPressed: () => setSheetState(
                      () => previewUrl = urlCtrl.text.isNotEmpty
                          ? urlCtrl.text
                          : null,
                    ),
                  ),
                ...
                onChanged: (v) =>
                    setSheetState(() => previewUrl = v.isNotEmpty ? v : null),
              ),
              const SizedBox(height: 16),
              SizedBox(
                width: double.infinity,
                      ...
                      : () async {
                          ...
                          try {
                            await ApiService().put(
                              ...
                              {'thumbnail': url, 'thumbnail_url': url},
                            );
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Expanded(
              child: Text(
                'Completions — ${module['title'] ?? 'Module'}',
                style: const TextStyle(
                  fontWeight: FontWeight.w700,
                  fontSize: 15,
                ),
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return ListTile(
                        ...
                        title: Text(
                          c['studentName']?.toString() ?? 'Student',
                          style: const TextStyle(
        ...
        content: Column(
          ...
          children: [
            ...
            if (meetingPassword.isNotEmpty)
              _infoTile('Password', meetingPassword),
        ...
        child: GridView.count(
          ...
          children: [
                ...
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) => CertificateTemplateSelectorScreen(
                      ...
                      instructorName:
                          _course?['instructor']?['name'] ?? 'Instructor',
                      courseId: widget.courseId,
                      currentTemplate: _certificateTemplate,
                      certificateReleased:
                          _course?['certificateReleased'] == true,
            ...
            _buildSettingsCard(
              ...
              rows: [
                _isFreeDisplay ? 'Showing as: Free' : 'Showing real price',
              ],
  ...
  String _creatorLabel(dynamic doc, String field) {
    ...
    final name =
        creator['name']?.toString() ?? creator['username']?.toString() ?? '';
  ...
  Widget _buildStandaloneSessionTile(dynamic session) {
    ...
    final scheduledAt =
        ...
        session['liveSessionDateTime']?.toString() ??
        '';
    ...
    final scheduledDt = scheduledAt.isNotEmpty
        ? _parseScheduledAt(scheduledAt)
        : null;
    final isLockedByTime =
        !isLive &&
        !isEnded &&
        scheduledDt != null &&
        scheduledDt.isAfter(DateTime.now());
            ...
            child: Column(
              ...
              children: [
                ...
                if (_creatorLabel(session, 'instructorId').isNotEmpty)
                  Text(
                    _creatorLabel(session, 'instructorId'),
                    style: const TextStyle(
                      fontSize: 11,
                      color: Color(0xFF94A3B8),
                    ),
              ...
              onTap: isLockedByTime
                  ? () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            'This session unlocks at ${_fmtDate(scheduledAt)}',
                          ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '$marks marks${dueDate.isNotEmpty ? ' · Due: ${_fmtDate(dueDate)}' : ''}',
                    style: const TextStyle(
                      fontSize: 12,
                      color: Color(0xFF64748B),
                    ),
                  ...
                  if (_creatorLabel(assignment, 'instructorId').isNotEmpty)
                    Text(
                      _creatorLabel(assignment, 'instructorId'),
                      style: const TextStyle(
                        fontSize: 11,
                        color: Color(0xFF94A3B8),
                      ),
  ...
  Widget _buildModuleCard(Map<String, dynamic> module, int index) {
    ...
    final linkedSessions = _liveSessions.where((s) {
      final m = s as Map;
      return m['linkedModuleId']?.toString() == moduleId &&
          m['linkedLessonId'] == null;
    }).toList();
  ...
  Widget _buildLessonItem(Map<String, dynamic> lesson, int index) {
    ...
    final hasRecording =
        ...
        lesson['recordingUrl'].toString().isNotEmpty;
    ...
    if (isLiveSession) {
      ...
      _activeLiveSessionForLesson ??= _liveSessions.cast<Map?>().firstWhere(
        (m) => m?['status']?.toString() == 'live',
        orElse: () => null,
      );
    }
    final effectiveStatus = _activeLiveSessionForLesson != null
        ? 'live'
        : sessionStatus;
    ...
    final scheduledAtRaw =
        ...
        lesson['liveSessionDateTime']?.toString() ??
        '';
    final lessonScheduledDt = scheduledAtRaw.isNotEmpty
        ? _parseScheduledAt(scheduledAtRaw)
        : null;
    final isLockedByTime =
        isLiveSession &&
        ...
        lessonScheduledDt != null &&
        lessonScheduledDt.isAfter(DateTime.now());
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      if (isLiveSession && lesson['scheduledAt'] != null) ...[
                        ...
                        Text(
                          _fmtDate(lesson['scheduledAt'].toString()),
                          style: const TextStyle(
                            fontSize: 12,
                            color: Color(0xFFF59E0B),
                          ),
                  ...
                  if (_creatorLabel(lesson, 'createdBy').isNotEmpty)
                    Text(
                      _creatorLabel(lesson, 'createdBy'),
                      style: const TextStyle(
                        fontSize: 11,
                        color: Color(0xFF94A3B8),
                      ),
                    ...
                    GestureDetector(
                      ...
                      onTap: () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              'This session unlocks at ${_fmtDate(scheduledAtRaw)}',
                            ),
                  ...
                  PopupMenuButton<String>(
                    ...
                    onSelected: (val) async {
                      if (val == 'cancel') {
                        ...
                        if (confirm == true) {
                          try {
                            await _lmsService.deleteSession(sessionId);
                            _loadCourse();
                          } catch (_) {}
                        }
                      }
                      if (val == 'delete_rec') {
                        ...
                        final confirm = await showDialog<bool>(
                          ...
                          builder: (ctx) => AlertDialog(
                            ...
                            content: const Text(
                              'Remove this recording? This cannot be undone.',
                            ),
                            actions: [
                              TextButton(
                                onPressed: () => Navigator.pop(ctx, false),
                        ...
                        if (confirm == true) {
                          try {
                            await _lmsService.updateSession(sessionId, {
                              'recordingUrl': '',
                            });
                            _loadCourse();
                          } catch (_) {}
  ...
  void _openRecording(dynamic lesson) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Recording is processing — uploading to Google Drive, check back shortly',
          ),
        ),
      );
    }
...
class _ModuleDialogState extends State<_ModuleDialog> {
  ...
  void initState() {
    ...
    if (widget.module != null) {
      ...
      _unlockDaysController.text = (days != null && days != 0)
          ? days.toString()
          : '';
      if (widget.module!['lessons'] != null) {
        _lessons.addAll(
          List<Map<String, dynamic>>.from(widget.module!['lessons']),
        );
      }
    }
  }
  ...
  void _addLesson({String defaultType = 'content'}) {
    if (defaultType == 'assignment') {
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) =>
              InstructorCreateAssignmentScreen(courseId: widget.courseId),
        ),
      ).then((result) {
    ...
    if (defaultType == 'quiz') {
      Navigator.push(
        ...
        MaterialPageRoute(
          ...
          builder: (_) => InstructorCreateQuizScreen(
            courseId: widget.courseId,
            moduleId: widget.module?['_id']?.toString(),
          ),
        ),
      ).then((result) {
        if (result is Map<String, dynamic>) {
  ...
  void _editLesson(int index) {
    ...
    if (ltype == 'assignment') {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => InstructorCreateAssignmentScreen(
            courseId: '',
    ...
    if (ltype == 'quiz') {
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) =>
              InstructorCreateQuizScreen(courseId: '', quizId: quizId),
        ),
      );
      return;
    }
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  ...
                  helperText:
                      'Day 0 from course start date when this module unlocks',
                ),
              ),
              const SizedBox(height: 16),
              Row(
        ...
        ElevatedButton(
          onPressed: () {
            ...
            Navigator.pop(context, {
              ...
              'unlockAfterDays':
                  int.tryParse(_unlockDaysController.text.trim()) ?? 0,
            });
          },
          style: ElevatedButton.styleFrom(
            backgroundColor: AppColors.primaryColor,
...
class _LessonDialogState extends State<_LessonDialog> {
  ...
  void initState() {
    ...
    if (widget.lesson != null) {
      ...
      _unlockDaysController.text = (widget.lesson!['unlockAfterDays'] ?? 0)
          .toString();
      _meetingLinkController.text =
          widget.lesson!['meetingLink']?.toString() ?? '';
    } else {
  ...
  Future<void> _uploadVideo() async {
    try {
      ...
      final signRes = await ApiService().get(
        '/upload/sign?folder=icare/lessons&resource_type=video',
      );
      ...
      if (signature.isEmpty)
        throw Exception('Could not get upload signature from server');
      ...
      final response = await dio2.post(
        ...
        onSendProgress: (sent, total) {
          if (total > 0 && mounted)
            setState(() => _uploadVideoProgress = sent / total);
        },
      );
  ...
  Future<void> _uploadDocument() async {
    try {
      ...
      if (res.statusCode != 200 || body['url'] == null) {
        throw Exception(
          body['message']?.toString() ?? 'Upload failed (${res.statusCode})',
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    final isCloudinaryVideo =
        currentUrl.contains('cloudinary.com') ||
        currentUrl.contains('.mp4') ||
        currentUrl.contains('.webm');

    return AlertDialog(
      title: Text(
        widget.lesson != null ? 'Edit Lesson' : 'Add Lesson',
                ...
                Container(
                  ...
                  child: const Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'Live session will be auto-recorded. Video + chat will be saved to this lesson automatically.',
                          style: TextStyle(
                            fontSize: 11,
        ...
        ElevatedButton(
          onPressed: () {
            ...
            widget.onSave({
              ...
              'videoUrl': _lessonType == 'content'
                  ? _videoUrlController.text.trim()
                  : '',
              'driveLink': _lessonType == 'content'
                  ? _driveLinkController.text.trim()
                  : '',
              'documentUrl': _lessonType == 'content'
                  ? (_documentUrl ?? '')
                  : '',
              'documentName': _lessonType == 'content'
                  ? (_documentName ?? '')
                  : '',
...
class _AssignmentPreviewDialog extends StatefulWidget {
  ...
  @override
  State<_AssignmentPreviewDialog> createState() =>
      _AssignmentPreviewDialogState();
}
...
class _AssignmentPreviewDialogState extends State<_AssignmentPreviewDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime? dueDate;
    if (_a['dueDate'] != null)
      dueDate = DateTime.tryParse(_a['dueDate'].toString());
              ...
              child: Row(
                children: [
                  Container(
                    padding: const EdgeInsets.all(8),
                    decoration: BoxDecoration(
                      color: const Color(0xFF0036BC).withValues(alpha: 0.12),
                      borderRadius: BorderRadius.circular(10),
                    ),
                    child: const Icon(
                      Icons.assignment_rounded,
  ...
  Widget _statChip(IconData icon, String label, Color fg, Color bg) =>
      Container(
...
class _QuizPreviewDialogState extends State<_QuizPreviewDialog> {
  ...
  Future<void> _fetch() async {
    try {
      ...
      if (mounted)
        setState(() {
          _q = Map<String, dynamic>.from(res['quiz'] ?? {});
          _loading = false;
        });
    } catch (_) {
      if (mounted) setState(() => _loading = false);
  ...
  Widget _buildQuestionCard(int qNum, Map<String, dynamic> q) {
    ...
    final correct =
        q['correctAnswer']?.toString() ?? q['answer']?.toString() ?? '';
