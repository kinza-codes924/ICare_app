...
import 'package:icare/utils/utils.dart'
    show buildProfileImageProvider, withDoctorTitle;
...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  Future<void> _loadPointsData() async {
    try {
      ...
      if (result['success'] == true && mounted) {
        setState(() {
          _totalPoints =
              (result['points'] ?? result['totalPoints'] ?? 0) as int;
          final history =
              result['history'] ??
              result['activities'] ??
              result['transactions'] ??
    ...
    try {
      ...
      if (mounted)
        setState(
          () => _prescriptionEmailEnabled =
              prefs.getBool('prescription_email_enabled') ?? true,
        );
    } catch (_) {}
  }
      ...
      if (mounted) {
        setState(() {
          ...
          if (healthTime != null) {
            ...
            if (parts.length == 2) {
              _healthCheckReminderTime = TimeOfDay(
                hour: int.tryParse(parts[0]) ?? 8,
                minute: int.tryParse(parts[1]) ?? 0,
              );
            }
          ...
          _reportDelivery =
              prefs.getString('report_delivery_method') ?? 'In-app';
          _courseNotificationsEnabled =
              prefs.getBool('course_notifications_enabled') ?? false;
        });
        ...
        if (_medReminderTime != null) {
          scheduleDailyReminder(
            ...
            'Time to take your medication. Stay on track!',
            _medReminderTime!.hour,
            _medReminderTime!.minute,
          );
        }
        if (_healthCheckReminderTime != null) {
          scheduleDailyReminder(
            'health_check',
            'Health Check Reminder ❤️',
            'Time to log your health metrics (BP, weight, etc.).',
            _healthCheckReminderTime!.hour,
            _healthCheckReminderTime!.minute,
          );
        }
      }
    } catch (_) {}
  ...
  Future<void> _loadNotifPrefs() async {
    try {
      ...
      if (cached.isNotEmpty && mounted)
        setState(() => _notifPrefs = {..._notifPrefs, ...cached});
      // Then sync from API
      ...
      if (data?['success'] == true && data?['preferences'] is Map) {
        ...
        if (updated.isNotEmpty && mounted)
          setState(() => _notifPrefs = {..._notifPrefs, ...updated});
        // Cache locally
  ...
  Future<void> _togglePrescriptionEmail(bool value) async {
    ...
    try {
      ...
      await ApiService().put('/auth/update-settings', {
        'prescriptionEmailEnabled': value,
      });
    } catch (_) {}
  }
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      if (mounted && result['success'] == true) {
        setState(
          () => _is2FAEnabled = result['settings']?['twoFactorEnabled'] == true,
        );
      }
    } catch (_) {}
    ...
    try {
      ...
      if (mounted && resp.data['success'] == true) {
        final emailEnabled =
            resp.data['user']?['prescriptionEmailEnabled'] as bool? ?? true;
      ...
      if (result['success'] && mounted) {
        ...
        setState(() {
          ...
          if (labPrefs != null) {
            ...
            _preferHomeSample =
                labPrefs['homeSampleCollection'] as bool? ?? _preferHomeSample;
          }
        });
      }
    } catch (_) {}
            ...
            ElevatedButton(
              onPressed: verifying
                  ...
                  : () async {
                      ...
                      final result = await _securityService.enable2FAWithOtp(
                      ...
                      if (result['success'] == true) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Google Authenticator 2FA enabled!'),
  ...
  Future<void> _toggleFaceId(bool value) async {
    if (value) {
      ...
      final path = await Navigator.push<String>(
        ...
        MaterialPageRoute(
          builder: (_) =>
              const FaceCaptureScreen(mode: FaceCaptureMode.register),
        ),
      );
          ...
          child: Form(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  const Text(
                    'Describe the issue you encountered and we\'ll investigate.',
    ...
    showDialog(
      ...
      builder: (dc) => StatefulBuilder(
        builder: (dc, setS) => AlertDialog(
          ...
          actions: [
            TextButton(
              onPressed: busy ? null : () => Navigator.pop(dc),
              child: const Text(
                'Cancel',
                style: TextStyle(color: Color(0xFF64748B)),
              ),
              ...
              ElevatedButton.icon(
                onPressed: (otpController.text.trim().length >= 4 && !busy)
                    ? () async {
                        ...
                        final err = await _doDeleteAccount(
                          ctx,
                          otpController.text.trim(),
                        );
                        if (err != null) {
    ...
    showDialog(
      ...
      builder: (dc) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              setState(() => _medicalConditions = c.text.trim());
              Navigator.pop(dc);
              ScaffoldMessenger.of(ctx).showSnackBar(
                const SnackBar(
                  content: Text('Medical conditions saved'),
                  backgroundColor: Colors.green,
                ),
              );
            },
    ...
    showDialog(
      ...
      builder: (dc) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              setState(() => _allergies = c.text.trim());
              Navigator.pop(dc);
              ScaffoldMessenger.of(ctx).showSnackBar(
                const SnackBar(
  ...
  void _showCurrentMedicationsDialog(BuildContext ctx) {
    Navigator.of(
      ctx,
    ).push(MaterialPageRoute(builder: (_) => const CurrentMedicationsPage()));
  }
  ...
  void _showHealthGoalsDialog(BuildContext ctx) {
    ...
    showDialog(
      ...
      builder: (dc) => AlertDialog(
        ...
        title: const Row(
          children: [
            Icon(Icons.flag_outlined, color: Color(0xFF10B981), size: 22),
            SizedBox(width: 10),
            Text(
              'Health Goals',
              style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              setState(() => _healthGoals = c.text.trim());
              Navigator.pop(dc);
              ScaffoldMessenger.of(ctx).showSnackBar(
                const SnackBar(
                  content: Text('Health goals saved'),
                  backgroundColor: Colors.green,
                ),
              );
            },
      ...
      builder: (dc) => StatefulBuilder(
        builder: (ctx2, setS) => AlertDialog(
              ...
              children: [
                ...
                ...List.generate(intervals.length, (i) {
                  ...
                  return Padding(
                    ...
                    child: InkWell(
                      onTap: () => setS(() => selected = intervals[i]),
                      borderRadius: BorderRadius.circular(10),
                      child: Container(
                        padding: const EdgeInsets.symmetric(
                          horizontal: 16,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                setState(() => _waterReminderMinutes = selected);
                Navigator.pop(dc);
                _setupWaterReminder(selected);
                ScaffoldMessenger.of(ctx).showSnackBar(
                  SnackBar(
                    content: Text(
                      'Water reminder set to every ${labels[intervals.indexOf(selected)]}',
                    ),
                    backgroundColor: Colors.green,
                  ),
                );
      ...
      if (permission == 'granted') {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Notifications blocked. Please enable in browser site settings.',
              ),
              backgroundColor: Colors.orange,
  ...
  Future<void> _loadBillingItems() async {
    ...
    try {
      ...
      for (final b in bookings) {
        ...
        final lab =
            (b['laboratory'] is Map ? b['laboratory']['labName'] : null) ??
            b['labName'] ??
            'Laboratory';
    ...
    try {
      ...
      for (final o in orders) {
        ...
        final itemNames = (o['items'] as List? ?? [])
            .take(3)
            .map((i) => i is Map ? (i['name'] ?? i['product_name'] ?? '') : '')
            .where((s) => s.toString().isNotEmpty)
            .join(', ');
        items.add({
          'id': o['_id'] ?? '',
          'type': 'medicine',
          'title': itemNames.isNotEmpty
              ? 'Medicine: $itemNames'
              : 'Medicine Order #${(o['order_number'] ?? '').toString().split('-').last}',
    ...
    showDialog(
      ...
      builder: (dc) => StatefulBuilder(
        builder: (_, setS) => AlertDialog(
          ...
          title: const Row(
            children: [
              Icon(Icons.translate_rounded, color: Color(0xFF64748B), size: 22),
              SizedBox(width: 10),
              Text(
                'Language / زبان',
                style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16),
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (lang['code'] == 'ur')
                                    const Text(
                                      'All app text will switch to Urdu',
                                      style: TextStyle(
                                        fontSize: 11,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                Navigator.pop(dc);
                final locale = selected == 'ur'
                    ? const Locale('ur')
                    : const Locale('en');
                await ctx.setLocale(locale);
                setState(
                  () =>
                      _selectedLanguage = selected == 'ur' ? 'اردو' : 'English',
                );
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
    ...
    showDialog(
      ...
      builder: (dc) => StatefulBuilder(
        builder: (ctx2, setS) => AlertDialog(
          ...
          title: const Row(
            children: [
              Icon(Icons.public_outlined, color: Color(0xFF64748B), size: 22),
              SizedBox(width: 10),
              Text(
                'Country & Region',
                style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16),
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                if (isPatient)
                  const Text(
                    'Currently available in Pakistan only. More countries coming soon!',
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                ScaffoldMessenger.of(ctx).showSnackBar(
                  SnackBar(
                    content: Text('Country set to $selected'),
                    backgroundColor: Colors.green,
                  ),
                );
              },
                  ...
                  return ListTile(
                    ...
                    title: Text(
                      itemNames.isNotEmpty
                          ? itemNames
                          : 'Order #${(o['order_number'] ?? o['_id'] ?? '').toString().split('-').last}',
                      style: const TextStyle(
                        fontSize: 13,
                        fontWeight: FontWeight.w600,
                      ),
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              const Text(
                'e.g. Leave at door, Ring bell twice, Call on arrival',
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                final prefs = await SharedPreferences.getInstance();
                await prefs.setString('delivery_instructions', text);
              } catch (_) {}
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: AppColors.primaryColor,
  ...
  void _downloadHealthData(BuildContext ctx) {
    showDialog(
      ...
      builder: (dc) => AlertDialog(
        ...
        title: const Row(
          children: [
            Icon(Icons.download_outlined, color: Color(0xFF8B5CF6), size: 22),
            SizedBox(width: 10),
            Text(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              Text(
                'A PDF will be generated with all your health records sorted by date.',
                style: TextStyle(fontSize: 13, color: Color(0xFF64748B)),
              ),
              SizedBox(height: 12),
              Text(
                'Includes: Consultations, Prescriptions, Lab Tests, Medicine Orders',
  ...
  Future<void> _generateHealthDataPdf(BuildContext ctx) async {
    try {
      ScaffoldMessenger.of(ctx).showSnackBar(
        const SnackBar(
          content: Text('Preparing your health data PDF...'),
      ...
      List orders = [];
      try {
        final r = await api.get('/appointments/getAppointments');
        appts = r.data['appointments'] as List? ?? [];
      } catch (_) {}
      try {
        final r = await api.get('/laboratories/bookings/my');
        labBookings = r.data['bookings'] as List? ?? [];
      } catch (_) {}
      try {
        final r = await api.get('/pharmacy/orders');
        orders = r.data['orders'] as List? ?? [];
      } catch (_) {}
                        ...
                        pw.Expanded(
                          child: pw.Column(
                            ...
                            children: [
                              ...
                              pw.Text(
                                'Date: ${fmt(a['date'] ?? a['createdAt'])}  |  Status: ${a['status'] ?? ''}',
                                style: const pw.TextStyle(
                                  fontSize: 9,
                                  color: PdfColors.grey,
                                ),
                        ...
                        pw.Expanded(
                          child: pw.Column(
                            ...
                            children: [
                              ...
                              pw.Text(
                                'Lab: $labName  |  Date: ${fmt(b['createdAt'] ?? b['test_date'])}',
                                style: const pw.TextStyle(
                                  fontSize: 9,
                                  color: PdfColors.grey,
                                ),
                        ...
                        pw.Expanded(
                          child: pw.Column(
                            ...
                            children: [
                              ...
                              pw.Text(
                                'Date: ${fmt(o['createdAt'])}  |  Status: $status',
                                style: const pw.TextStyle(
                                  fontSize: 9,
                                  color: PdfColors.grey,
                                ),
      ...
      await Printing.layoutPdf(
        ...
        name:
            'iCare_Health_Data_${patientName.replaceAll(' ', '_')}_$dateStr.pdf',
      );
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(ctx).showSnackBar(
                      ...
                      child: Row(
                        children: [
                          ...
                          IconButton(
                            ...
                            onPressed: () {
                              setState(() => _savedCards.removeAt(i));
                              setS(() {});
                            },
                          ),
                        ],
    ...
    showDialog(
      ...
      builder: (dc) => StatefulBuilder(
        builder: (dc2, setS) => AlertDialog(
          ...
          title: const Row(
            children: [
              Icon(Icons.add_card_outlined, color: Color(0xFF10B981), size: 22),
              SizedBox(width: 10),
              Text(
                'Add Payment Method',
                style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                ScaffoldMessenger.of(ctx).showSnackBar(
                  SnackBar(
                    content: Text('$selectedType card added successfully'),
                    backgroundColor: Colors.green,
                  ),
                );
              },
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return ListTile(
                      ...
                      leading: Container(
                        ...
                        child: Icon(
                          item['icon'] as IconData? ?? Icons.receipt_rounded,
                          color:
                              item['color'] as Color? ??
                              const Color(0xFF10B981),
                          size: 18,
                      ...
                      trailing: Row(
                        ...
                        children: [
                          ...
                          InkWell(
                            onTap: () {
                              Navigator.pop(dc);
                              _downloadSingleReceipt(ctx, item);
                            },
                            borderRadius: BorderRadius.circular(6),
                            child: const Padding(
  ...
  Future<void> _downloadSingleReceipt(
    BuildContext ctx,
    ...
    try {
      ...
      try {
        final dt = DateTime.parse(rawDate);
        txnDate = '${dt.day}/${dt.month}/${dt.year}';
      } catch (_) {
        txnDate = rawDate;
      }
      final amount = item['amount'] as String? ?? '';
                ...
                children: [
                  ...
                  pw.Column(
                    ...
                    children: [
                      ...
                      pw.SizedBox(height: 4),
                      pw.Text(
                        txnDate.isNotEmpty ? txnDate : 'N/A',
      ...
      await Printing.layoutPdf(
        onLayout: (_) async => pdf.save(),
        name:
            'iCare_Receipt_${(item['title'] as String? ?? 'receipt').replaceAll(' ', '_')}.pdf',
      );
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(ctx).showSnackBar(
  ...
  Future<void> _setHealthCheckReminder(TimeOfDay? time) async {
    ...
    try {
      ...
      if (time != null) {
        await prefs.setString(
          'health_check_reminder_time',
          '${time.hour}:${time.minute}',
        );
        await ReminderService().createReminder({
          'title': 'Log your health metrics',
          'type': 'health_check',
          'time':
              '${time.hour.toString().padLeft(2, '0')}:${time.minute.toString().padLeft(2, '0')}',
          'isRecurring': true,
  ...
  void _showFeeDialog(BuildContext ctx) {
    final ctrl = TextEditingController(
      text: _consultationFee != null
          ? _consultationFee!.toInt().toString()
          : '',
    );
    bool saving = false;
    showDialog(
              ...
              onPressed: saving
                  ...
                  : () async {
                      ...
                      try {
                        ...
                        if (mounted) setState(() => _consultationFee = fee);
                        if (dc2.mounted) Navigator.pop(dc2);
                        if (ctx.mounted)
                          ScaffoldMessenger.of(ctx).showSnackBar(
                            SnackBar(
                              content: Text(
                                'Consultation fee set to PKR ${fee.toInt()}',
                              ),
                              backgroundColor: Colors.green,
                            ),
                          );
  ...
  Widget build(BuildContext context) {
    ...
    final params = _SettingsLayoutParams(
      ...
      prescriptionEmailEnabled: _prescriptionEmailEnabled,
      ...
      savedAddressSubtitle: _savedAddresses.isEmpty
          ? 'Tap to add'
          : '${_savedAddresses.length} address${_savedAddresses.length == 1 ? '' : 'es'} saved',
      savedPaymentMethods: _savedPaymentMethods,
      billingHistory: _billingHistory,
      totalPoints: _totalPoints,
      pointsHistory: _pointsHistory,
      medReminderTime: _medReminderTime,
      healthCheckReminderTime: _healthCheckReminderTime,
      appointmentRemindersEnabled: _appointmentRemindersEnabled,
      preferHomeSample: _preferHomeSample,
      reportDelivery: _reportDelivery,
      onToggle2FA: _toggle2FA,
      onToggleBiometrics: _toggleBiometrics,
      onToggleFaceId: _toggleFaceId,
      ...
      onShowMedicalConditions: _showMedicalConditionsDialog,
      onShowAllergies: _showAllergiesDialog,
      onShowCurrentMedications: _showCurrentMedicationsDialog,
      onShowHealthGoals: _showHealthGoalsDialog,
      onShowWaterReminder: _showWaterReminderDialog,
      onShowLanguage: _showLanguageDialog,
      onShowCountryRegion: _showCountryRegionDialog,
      onShowDeliveryAddress: _showDeliveryAddressesDialog,
      onDownloadHealthData: _downloadHealthData,
      onShowPaymentMethods: _showPaymentMethodsDialog,
      onShowBillingHistory: _showBillingHistoryDialog,
      onShowOrderHistory: _showOrderHistoryDialog,
      onShowDeliveryPreferences: _showDeliveryPreferencesDialog,
      onSetMedReminder: _setMedReminder,
      onSetHealthCheckReminder: _setHealthCheckReminder,
      onToggleAppointmentReminders: _toggleAppointmentReminders,
      onTogglePreferHomeSample: _togglePreferHomeSample,
      onSetReportDelivery: _setReportDelivery,
...
class _SettingsLayoutParams {
  ...
  final void Function(BuildContext) onReportIssue,
      onDeleteAccount,
      onShowFeeDialog;
  final void Function(BuildContext) onShowMedicalConditions,
      onShowAllergies,
      onShowCurrentMedications;
  final void Function(BuildContext) onShowHealthGoals,
      onShowWaterReminder,
      onShowLanguage;
  ...
  final void Function(BuildContext) onShowOrderHistory,
      onShowDeliveryPreferences;
  ...
  const _SettingsLayoutParams({
    ...
    required this.prescriptionEmailEnabled,
    ...
    required this.waterReminderMinutes,
    ...
    required this.onShowFeeDialog,
    required this.onShowMedicalConditions,
    required this.onShowAllergies,
    required this.onShowCurrentMedications,
    required this.onShowHealthGoals,
    required this.onShowWaterReminder,
    required this.onShowLanguage,
    required this.onShowCountryRegion,
    required this.onShowDeliveryAddress,
    required this.onDownloadHealthData,
    required this.onShowPaymentMethods,
    required this.onShowBillingHistory,
    required this.onShowOrderHistory,
    required this.onShowDeliveryPreferences,
    required this.onSetMedReminder,
    required this.onSetHealthCheckReminder,
    required this.onToggleAppointmentReminders,
    required this.onTogglePreferHomeSample,
    required this.onSetReportDelivery,
    required this.courseNotificationsEnabled,
    required this.onToggleCourseNotifications,
    required this.notifPrefs,
    required this.onSaveNotifPref,
...
void showInviteFriendsDialog(BuildContext context) {
  const playStoreUrl =
      'https://play.google.com/store/apps/details?id=com.cartzlinkv2.icare';
  const message =
      "I'm using iCare for online doctor consultations, lab tests and medicines. Download it here: $playStoreUrl";
  showDialog(
    context: context,
    builder: (ctx) => AlertDialog(
      ...
      content: Column(
        ...
        children: [
          const Text(
            'Share iCare with friends and family so they can book doctors, lab tests and medicines from their phone.',
            style: TextStyle(
              fontSize: 13,
              color: Color(0xFF475569),
              height: 1.5,
...
class _WebSettingsLayout extends StatelessWidget {
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (p.isDoctor) ...[
                    _doctorProfessionalCard(context),
                    const SizedBox(height: 24),
                  ],
                  if (p.isLaboratory) ...[
                    _labManagementCard(context),
                    const SizedBox(height: 24),
                  ],
                  if (p.isPatient) ...[
                    _healthProfile(context),
                    const SizedBox(height: 24),
                  ],
                  ...
                  if (p.isPatient || p.isStudent || p.isInstructor) ...[
                    _learningCard(context),
                    const SizedBox(height: 24),
                  ],
                  _securityCard(context), const SizedBox(height: 24),
                  if (p.isDoctor) ...[
                    _notificationSettingsCard(context),
                    const SizedBox(height: 24),
                  ],
                      ...
                      if (img != null)
                        return Image(
                          ...
                          errorBuilder: (_, _, _) => Text(
                            (p.user?.name ?? 'U').substring(0, 1).toUpperCase(),
                            style: const TextStyle(
                              fontSize: 28,
        ...
        child: Column(
          ...
          children: [
            ...
            _settingsTile(
              ...
              subtitle: p.medicalConditions.isEmpty
                  ? 'Tap to add'
                  : p.medicalConditions,
              onTap: () => p.onShowMedicalConditions(context),
            ),
            ...
            _settingsTile(
              ...
              title: 'Allergies',
              subtitle: p.allergies.isEmpty ? 'Tap to add' : p.allergies,
              onTap: () => p.onShowAllergies(context),
            ),
            const Divider(height: 1),
            _settingsTile(
              icon: Icons.medication_outlined,
              ...
              subtitle: p.currentMedications.isEmpty
                  ? 'Tap to add'
                  : p.currentMedications,
              onTap: () => p.onShowCurrentMedications(context),
            ),
            const Divider(height: 1),
            _settingsTile(
            ...
            _settingsTile(
              ...
              title: 'Health Goals',
              subtitle: p.healthGoals.isEmpty
                  ? 'Tap to set goals'
                  : p.healthGoals,
              onTap: () => p.onShowHealthGoals(context),
            ),
          ],
        ),
      ),
          ...
          children: [
            ...
            if (p.isLaboratory) ...[
              _switchTile(
                ...
                subtitle: 'Required â€¢ Cannot be turned off',
                value: true,
                onChanged: (_) {},
              ),
              const Divider(height: 1),
              _switchTile(
                ...
                subtitle: 'Updates on sample pickup & processing',
                value: p.notifPrefs['delivery_updates'] ?? true,
                onChanged: (v) => p.onSaveNotifPref('delivery_updates', v),
              ),
              const Divider(height: 1),
              _switchTile(
                icon: Icons.upload_file_outlined,
                title: 'Result Upload Reminders',
                subtitle: 'Reminders to upload pending test results',
                value: p.notifPrefs['booking_updates'] ?? true,
                onChanged: (v) => p.onSaveNotifPref('booking_updates', v),
              ),
              const Divider(height: 1),
              _switchTile(
                icon: Icons.warning_amber_outlined,
                title: 'System Alerts',
                subtitle: 'Platform & maintenance notifications',
                value: p.notifPrefs['system_alerts'] ?? true,
                onChanged: (v) => p.onSaveNotifPref('system_alerts', v),
              ),
              const Divider(height: 1),
              _switchTile(
                ...
                value: p.notifPrefs['sound_notifications'] ?? true,
                onChanged: (v) => p.onSaveNotifPref('sound_notifications', v),
              ),
              ...
              _switchTile(
                ...
                subtitle: 'Notify when order is out for delivery',
                value: p.notifPrefs['order_dispatched'] ?? true,
                onChanged: (v) => p.onSaveNotifPref('order_dispatched', v),
              ),
              const Divider(height: 1),
              _switchTile(
              ...
              _switchTile(
                ...
                subtitle: 'Special deals & health tips',
                value: p.notifPrefs['promotions'] ?? false,
                onChanged: (v) => p.onSaveNotifPref('promotions', v),
              ),
              const Divider(height: 1),
              _switchTile(
                icon: Icons.volume_up_outlined,
              ...
              _switchTile(
                ...
                subtitle: 'Appointment confirmations & changes',
                value: p.notifPrefs['booking_updates'] ?? true,
                onChanged: (v) => p.onSaveNotifPref('booking_updates', v),
              ),
              const Divider(height: 1),
              _switchTile(
                ...
                subtitle: 'Messages from providers',
                value: p.notifPrefs['doctor_messages'] ?? true,
                onChanged: (v) => p.onSaveNotifPref('doctor_messages', v),
              ),
              const Divider(height: 1),
              _switchTile(
                icon: Icons.local_offer_outlined,
              ...
              if (p.isPatient) ...[
                ...
                _switchTile(
                  ...
                  subtitle:
                      'Automatically email prescriptions after consultation',
                  value: p.prescriptionEmailEnabled,
                  onChanged: p.onTogglePrescriptionEmail,
                ),
              ],
            ],
          ],
        ...
        child: Column(
          ...
          children: [
            ...
            _settingsTile(
              ...
              subtitle: label,
              onTap: () => p.onShowWaterReminder(context),
            ),
          ],
        ),
      ),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sectionLabel('Reminders & Notifications'),
            const SizedBox(height: 16),
            _settingsTile(
              icon: Icons.medication_outlined,
              iconColor: const Color(0xFF8B5CF6),
              title: 'Medication Reminder',
              subtitle: fmtTime(p.medReminderTime),
              onTap: () async {
                final picked = await showTimePicker(
                  context: context,
                  initialTime:
                      p.medReminderTime ?? const TimeOfDay(hour: 8, minute: 0),
                );
            ...
            _settingsTile(
              ...
              subtitle: fmtTime(p.healthCheckReminderTime),
              onTap: () async {
                final picked = await showTimePicker(
                  context: context,
                  initialTime:
                      p.healthCheckReminderTime ??
            ...
            _switchTile(
              ...
              value: p.appointmentRemindersEnabled,
              onChanged: p.onToggleAppointmentReminders,
            ),
          ],
        ),
      ),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sectionLabel('Diagnostics Settings'),
            const SizedBox(height: 16),
            _settingsTile(
              icon: Icons.science_outlined,
              iconColor: const Color(0xFF3B82F6),
              title: 'Test History',
              subtitle: 'View past lab bookings',
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const PatientLabOrdersScreen(),
                ),
              ),
            ),
            const Divider(height: 1),
            _switchTile(
              ...
              subtitle: 'Prefer home sample collection',
              value: p.preferHomeSample,
              onChanged: p.onTogglePreferHomeSample,
            ),
            const Divider(height: 1),
            ListTile(
        ...
        child: Column(
          ...
          children: [
            ...
            _settingsTile(
              ...
              iconColor: const Color(0xFFF59E0B),
              title: 'Reward Points',
              subtitle: '${p.totalPoints} pts total',
              onTap: () => _showRewardPointsDialog(context),
            ),
            const Divider(height: 1),
            _settingsTile(
              ...
              subtitle: p.pointsHistory.isEmpty
                  ? 'No activity yet'
                  : '${p.pointsHistory.length} activities',
              onTap: () => _showRewardHistoryDialog(context),
            ),
            const Divider(height: 1),
            _settingsTile(
              ...
              onTap: () => _showRedemptionHistoryDialog(context),
            ),
          ],
        ),
      ),
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              Text(
                'â‰ˆ PKR ${(p.totalPoints * _kPointToPkr).toStringAsFixed(2)}',
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () {
                    Navigator.pop(dc);
                    _showRedeemRewardsDialog(context);
                  },
  ...
  void _showRedeemRewardsDialog(BuildContext context) {
    final rewards = [
      ...
      {
        'id': 'lab_test_discount',
        'title': 'Lab Test Discount (20%)',
        'cost': 500,
        'icon': Icons.science_outlined,
        'color': const Color(0xFF3B82F6),
      },
    ...
    showDialog(
      ...
      builder: (dc) => StatefulBuilder(
        builder: (dc2, setS) => AlertDialog(
          ...
          title: const Row(
            children: [
              Icon(Icons.redeem_rounded, color: Color(0xFF10B981), size: 24),
              SizedBox(width: 10),
              Text(
                'Redeem Points',
                style: TextStyle(fontWeight: FontWeight.w700, fontSize: 18),
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                          ...
                          ElevatedButton(
                            onPressed: hasEnough
                                ? () async {
                                    Navigator.pop(dc2);
                                    try {
                                      final result = await gamSvc.redeemReward(
                                        r['id'] as String,
                                      );
                                      if (context.mounted)
                                        ScaffoldMessenger.of(
                ...
                const Divider(),
                const Text(
                  '1 pt = PKR 0.01 â€¢ Rate may change by platform',
  ...
  void _showRedemptionHistoryDialog(BuildContext context) {
    ...
    showDialog(
      context: context,
      builder: (dc) => FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (_, snap) {
          ...
          if (snap.hasData && snap.data!['success'] == true) {
            ...
            redemptions =
                (data['redemptions'] ?? data['redeemedRewards'] ?? [])
                    as List<dynamic>;
          }
          return AlertDialog(
            shape: RoundedRectangleBorder(
        ...
        child: Column(
          ...
          children: [
            ...
            _settingsTile(
              ...
              subtitle: 'Export all consultations & records',
              onTap: () => p.onDownloadHealthData(context),
            ),
          ],
        ),
      ),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sectionLabel('Payment & Subscription'),
            const SizedBox(height: 16),
            _settingsTile(
              icon: Icons.credit_card_outlined,
              iconColor: const Color(0xFF10B981),
              title: 'Saved Payment Methods',
              subtitle: p.savedPaymentMethods.isEmpty
                  ? 'No methods saved'
                  : '${p.savedPaymentMethods.length} method(s)',
              onTap: () => p.onShowPaymentMethods(context),
            ),
            const Divider(height: 1),
            _settingsTile(
              icon: Icons.receipt_long_outlined,
              ...
              subtitle: p.billingHistory.isEmpty
                  ? 'View transactions'
                  : '${p.billingHistory.length} transaction(s)',
              onTap: () => p.onShowBillingHistory(context),
            ),
          ],
        ),
      ),
          ...
          children: [
            ...
            _settingsTile(
              ...
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const PaymentHistoryScreen(role: 'Student'),
                ),
              ),
            ),
          ],
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sectionLabel('Contact & Legal'),
            const SizedBox(height: 16),
            _settingsTile(
              icon: Icons.person_add_alt_1_outlined,
              iconColor: const Color(0xFF10B981),
              title: 'Invite Friends',
              subtitle: 'Share the iCare app',
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sectionLabel('Pharmacy Settings'),
            const SizedBox(height: 16),
            _settingsTile(
              icon: Icons.location_on_outlined,
              iconColor: const Color(0xFF10B981),
              title: 'Saved Addresses',
              subtitle: p.savedAddressSubtitle,
              onTap: () => p.onShowDeliveryAddress(context),
            ),
            const Divider(height: 1),
            _settingsTile(
              ...
              iconColor: const Color(0xFF3B82F6),
              title: 'Order History',
              subtitle: 'View all pharmacy orders',
              onTap: () => p.onShowOrderHistory(context),
            ),
            const Divider(height: 1),
            _settingsTile(
              icon: Icons.local_shipping_outlined,
              ...
              subtitle: 'Set delivery instructions',
              onTap: () => p.onShowDeliveryPreferences(context),
            ),
          ],
        ),
      ),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sectionLabel('Lab Management'),
            const SizedBox(height: 16),
            _settingsTile(
              icon: Icons.biotech_outlined,
              iconColor: const Color(0xFF0036BC),
              title: 'Lab Profile & Settings',
              subtitle: 'Update lab info, hours & services',
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => const LabSettingsScreen()),
              ),
            ),
            const Divider(height: 1),
            _settingsTile(
              ...
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => const LabTestsManagement()),
              ),
            ),
          ],
        ),
          ...
          children: [
            ...
            _settingsTile(
              ...
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const Courses(myPurchased: true),
                ),
              ),
            ),
            const Divider(height: 1),
            ...
            _settingsTile(
              ...
              subtitle: 'View your course completion',
              // Was pushing the whole StudentLmsDashboard, so "Progress
              // Tracking" opened iCare Academy instead of any progress.
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
            ...
            _switchTile(
              ...
              subtitle: 'Get notified about new offerings',
              value: p.courseNotificationsEnabled,
              onChanged: p.onToggleCourseNotifications,
            ),
          ],
        ),
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            _settingsTile(
              ...
              subtitle: 'Review recent login sessions',
              onTap: () => Navigator.push(
                context,
            ...
            if (p.biometricAvailable) ...[
              ...
              _switchTile(
                ...
                subtitle: p.isBiometricEnabled
                    ? 'Tap to disable fingerprint login'
                    : 'Enable fingerprint login',
                value: p.isBiometricEnabled,
                onChanged: p.onToggleBiometrics,
              ),
            ],
            // Face ID (camera-based). Phone only - hidden on web at the
            // client's request; the camera pipeline does not work there.
            if (!kIsWeb) ...[
              const Divider(height: 1),
              _switchTile(
                icon: Icons.face_retouching_natural,
                title: 'Face ID Sign-In',
                subtitle: p.isFaceEnabled
                    ? 'Tap to disable Face ID login'
                    : 'Enable camera Face ID login (works on all phones)',
                value: p.isFaceEnabled,
                onChanged: p.onToggleFaceId,
              ),
            ],
            ...
            _switchTile(
              ...
              subtitle: p.is2FAEnabled ? 'Enabled' : 'Extra layer of security',
              value: p.is2FAEnabled,
              onChanged: p.onToggle2FA,
            ),
          ],
        ),
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            _settingsTile(
              ...
              iconColor: const Color(0xFF64748B),
              title: 'Language',
              subtitle: p.selectedLanguage,
              onTap: () => p.onShowLanguage(context),
            ),
            const Divider(height: 1),
            _settingsTile(
              ...
              subtitle: p.selectedCountry,
              onTap: () => p.onShowCountryRegion(context),
            ),
          ],
        ),
      ),
              ...
              return Column(
                children: [
                  ListTile(
                    ...
                    subtitle: Text(
                      isOn ? 'Currently tracking'.tr() : 'Not tracking'.tr(),
                      style: TextStyle(
                        fontSize: 12,
                        color: isOn
                            ? const Color(0xFF10B981)
                    ...
                    trailing: Switch(
                      value: isOn,
                      onChanged: (v) => p.onTrackerToggle(key, v),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _sectionLabel('Health Mode'),
            const SizedBox(height: 16),
            ...conditions.map((c) {
              final sel = p.selectedConditions.contains(c);
              return Padding(
                ...
                child: Column(
                  children: [
                    _switchTile(
                      icon: Icons.monitor_heart_outlined,
                      title: c,
                      subtitle: sel ? 'Active' : 'Tap to enable',
                      value: sel,
                      onChanged: (v) => p.onHealthModeToggle(c, v),
                    ),
                    if (c != conditions.last) const Divider(height: 1),
                  ],
                ),
              );
            }),
        ...
        child: Column(
          ...
          children: [
            ...
            _settingsTile(
              ...
              iconColor: const Color(0xFF64748B),
              title: 'About Us',
              subtitle: 'Learn more about iCare',
              onTap: () async {
                try {
                  await launchUrl(
                    Uri.parse('https://rmhealthsolutions.co.uk/about-us'),
                    mode: LaunchMode.externalApplication,
                  );
                } catch (_) {}
              },
            ...
            if (showDrugInfo) ...[
              ...
              _settingsTile(
                icon: Icons.verified_outlined,
                iconColor: const Color(0xFF10B981),
                title: 'DRAP Guidelines',
                subtitle: 'Drug Regulatory Authority of Pakistan',
                onTap: () async {
                  try {
                    await launchUrl(
                      Uri.parse('https://www.dra.gov.pk'),
                      mode: LaunchMode.externalApplication,
                    );
                  } catch (_) {}
                },
              ...
              _settingsTile(
                ...
                iconColor: const Color(0xFF3B82F6),
                title: 'Drug Policy',
                subtitle: 'National drug laws & regulations',
                onTap: () async {
                  try {
                    await launchUrl(
                      Uri.parse(
                        'https://www.dra.gov.pk/therapeutic-goods/drugs/pharmaceuticals/',
                      ),
                      mode: LaunchMode.externalApplication,
                    );
                  } catch (_) {}
...
class _MobileSettingsLayout extends StatelessWidget {
              ...
              return Column(
                children: [
                  ListTile(
                    ...
                    subtitle: Text(
                      isOn ? 'Tracking'.tr() : 'Not tracking'.tr(),
                      style: TextStyle(
                        fontSize: 11,
                        color: isOn
                            ? const Color(0xFF10B981)
          ...
          children: [
            ...
            if (p.isLaboratory) ...[
              ...
              _switchTile(
                icon: Icons.upload_file_outlined,
                title: 'Result Upload Reminders',
                subtitle: 'Reminders to upload pending results',
                value: p.notifPrefs['booking_updates'] ?? true,
                onChanged: (v) => p.onSaveNotifPref('booking_updates', v),
              ),
              const Divider(height: 1),
              ...
              if (p.isPatient) ...[
                ...
                _switchTile(
                  icon: Icons.email_outlined,
                  title: 'Send Prescription to Email',
                  subtitle: 'Auto email prescriptions after consultation',
                  value: p.prescriptionEmailEnabled,
                  onChanged: p.onTogglePrescriptionEmail,
                ),
              ],
  ...
  Widget _remindersCard(BuildContext context) {
    ...
    String formatTime(TimeOfDay? t) =>
        t == null ? 'Not set' : t.format(context);
    return Card(
      elevation: 0,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _settingsTile(
              ...
              iconColor: const Color(0xFF8B5CF6),
              title: 'Medication Reminder',
              subtitle: formatTime(p.medReminderTime),
              onTap: () async {
                final picked = await showTimePicker(
                  context: context,
                  initialTime:
            ...
            _settingsTile(
              ...
              iconColor: const Color(0xFFEF4444),
              title: 'Health Check Reminder',
              subtitle: formatTime(p.healthCheckReminderTime),
              onTap: () async {
                final picked = await showTimePicker(
                  context: context,
                  initialTime:
        ...
        child: Column(
          ...
          children: [
            ...
            _settingsTile(
              ...
              iconColor: const Color(0xFF8B5CF6),
              title: 'Progress Tracking',
              subtitle: 'View course completion',
              // Same fix as the other Progress Tracking tile above - this one
              // is the web layout's copy.
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const ProgressTrackingScreen(),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Face ID (camera-based). Phone only - see the note above.
            if (!kIsWeb) ...[
              const Divider(height: 1),
              _switchTile(
                icon: Icons.face_retouching_natural,
            ...
            _switchTile(
              icon: Icons.verified_user_outlined,
              title: '2FA',
              subtitle: p.is2FAEnabled ? 'Enabled' : 'Extra security',
              value: p.is2FAEnabled,
              onChanged: p.onToggle2FA,
            ),
          ],
  ...
  Widget _sectionLabel(String title) {
    return Row(
      children: [
        const Icon(Icons.circle, size: 7, color: AppColors.primaryColor),
...
class _ProfileEditCardState extends ConsumerState<_ProfileEditCard> {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (data is Map && data['user'] is Map) {
          userMap = Map<String, dynamic>.from(data['user'] as Map);
            ...
            (data.containsKey('_id') || data.containsKey('id'))) {
  ...
  Widget build(BuildContext context) {
    ...
    final age = _ageCtrl.text.isNotEmpty
        ? _ageCtrl.text
        : (u?.age ?? 'Not set');
      ...
      child: Padding(
        ...
        child: Form(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // Every other card in Settings carries a section label; this one
              // was the only one without ("iske upar to PROFILE ki HEADING
              // daalo na kahin pe - baaki sab pe heading di hui hai").
              Row(
                children: [
              ...
              Row(
                ...
                children: [
                  ...
                  if (!_editMode)
                    TextButton.icon(
                      // Settings > Edit and the avatar in the top-right must
                      // land on the same screen. Only the doctor case did;
                      // everyone else got an inline edit here and the full
                      // setup page from the avatar - two different layouts for
                      // the same job ("yeh dono profile par SAME hona chahiye
                      // - EK HI LAYOUT hona chahiye").
                      onPressed: () {
                        ...
                        } else if (widget.p.isStudent) {
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (_) => const StudentProfileSetup(),
                            ),
                          );
                        } else if (widget.p.isInstructor) {
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (_) => InstructorProfileSetupScreen(),
                            ),
                          );
                        } else {
              ...
              if (_editMode) ...[
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: ['Male', 'Female', 'Other']
                      .map(
                        (g) => DropdownMenuItem(value: g, child: Text(g.tr())),
                      )
                      .toList(),
